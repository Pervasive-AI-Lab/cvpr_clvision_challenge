...
class EpochData(Dataset):
    def __init__(self, x, y, istraining):
        super().__init__()
        self.x = x
        self.y = y
        self.augment_list = [
            ...
            transforms.RandomApply([transforms.RandomResizedCrop((224, 224), scale=(0.9, 1.0))], p=0.5),
            transforms.RandomApply([transforms.RandomAffine(degrees=45, translate=(0.2, 0.2), scale=(0.8, 1.2), shear=16)], p=0.5)
        ] if istraining else []
        self.preprocess = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.RandomOrder(self.augment_list),
            transforms.ToTensor(),
...
def train_net(optimizer, model, criterion, mb_size, x, y, t,
              ...
              lr_scheduler=None, fixed_BN=False):
    """
    ...
    # ribo
    # x = CORE50.get_batch_from_paths(x).astype(np.float32)
    # if preproc:
    #     x = preproc(x)
    ...
    dataset = EpochData(x, y, istraining=True)
...
def test_multitask(
        model, test_set, mb_size, preproc=None, use_cuda=True, multi_heads=[], multi_models=[], verbose=True):
    """
    ...
    for (x, y), t in test_set:
        ...
        # if preproc:
        #     x = preproc(x)
        dataset = EpochData(x, y, istraining=False)
        ...
        if multi_models != [] and len(multi_models) > t:
            if verbose:
                print("Using model: ", t)
            model = multi_models[t]
            model.eval()
