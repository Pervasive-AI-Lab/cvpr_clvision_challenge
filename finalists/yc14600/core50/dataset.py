...
class CORE50(object):
    ...
    new2old_names = {'ni': 'ni', 'multi-task-nc': 'nc', 'nic': 'nicv2_391', 'offline':'offline'}
    nbatch = {
        'ni': 8,
        'nc': 9,
        'nicv2_391': 391,
    ...
    def __init__(self, root='', preload=False, scenario='ni', cumul=False,
                 run=0, start_batch=0, task_sep=False,train_size=-1):
        """" Initialize Object """
        ...
        if self.scenario == 'offline': #and 'offline' not in self.LUP.keys():
            off_lup,off_lbl = [],[]
            for r in range(len(self.LUP['ni'])):
                lup = np.concatenate(self.LUP['ni'][r][:-1])
                lbl = np.concatenate(self.labels['ni'][r][:-1])
                ##### cutoff for memory#####
                if train_size > 0:
                    lup = lup[:train_size]
                    lbl = lbl[:train_size]

                off_lup.append([lup,self.LUP['ni'][r][-1]])
                off_lbl.append([lbl,self.labels['ni'][r][-1]])

            self.LUP.update({'offline':off_lup})
            self.labels.update({'offline':off_lbl})
    ...
    def get_full_test_set(self):
        ...
        filelist_path = os.path.join(self.root, "test_filelist.txt")
        filelist_tlabeled_path = os.path.join(self.root, "test_filelist_tlabeled.txt")
        test_img_dir = os.path.join(self.root,  "core50_challenge_test")
    ...
    def get_batch_from_paths(paths, compress=False, snap_dir='',
        ...
        if compress:
            file_path = os.path.join(snap_dir, hexdigest, ".npz")
            ...
            x_file_path = os.path.join(snap_dir, hexdigest, "_x.bin")
