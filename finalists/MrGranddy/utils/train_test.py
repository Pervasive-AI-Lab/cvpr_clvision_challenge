...
def train_net(optimizer, model, criterion, mb_size, x, y, t,
    ...
    for ep in range(train_ep):
        ...
        for it in range(it_x_ep):
            ...
            x_mb = warp(maybe_cuda(train_x[start:end], use_cuda=use_cuda))
...
def test_multitask(
    ...
    for (x, y), t in test_set:
        ...
        if multi_heads != [] and len(multi_heads) > t:
            ...
            if verbose:
                print("Using head: ", t)
            with torch.no_grad():
                model._fc.weight.copy_(multi_heads[t].weight)
                model._fc.bias.copy_(multi_heads[t].bias)
    ...
    if multi_heads:
        ...
        with torch.no_grad():
            model._fc.weight.fill_(0)
            model._fc.bias.fill_(0)
